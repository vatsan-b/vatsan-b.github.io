...
class RenderedGalleryContractTests(unittest.TestCase):
    ...
    def test_previews_are_lazy_and_responsive(self):
        # Image elements may be emitted as JavaScript templates, so inspect the
        # serialized img tags as well as ordinary parsed markup.
        image_tags = re.findall(r"<img\b[^>]*>", self.html, flags=re.IGNORECASE)
        preview_tags = [tag for tag in image_tags if re.search(r"loading=[\"']lazy[\"']", tag, re.I)]
        self.assertGreaterEqual(len(preview_tags), 2, "both galleries should create lazy previews")
        for tag in preview_tags:
            self.assertRegex(tag, r"\bsrcset=[\"']", "lazy gallery previews must be responsive")
            self.assertRegex(tag, r"\bsizes=[\"']", "responsive previews must provide sizes")

    def test_photoswipe_markup_and_shared_configuration_are_present(self):
        self.assertIn("photoswipe", self.lower_html)
        self.assertRegex(self.lower_html, r"data-pswp-(?:width|w)\b")
        self.assertRegex(self.lower_html, r"data-pswp-(?:height|h)\b")
        self.assertRegex(self.html, r"PhotoSwipeLightbox\s*\(")
        self.assertEqual(
            len(re.findall(r"PhotoSwipeLightbox\s*\(", self.html)),
            1,
